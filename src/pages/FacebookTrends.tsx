...
import {
  Facebook, TrendingUp, Activity, Lightbulb, BarChart3,
...
export default function FacebookTrends() {
  const { data: trends = [] } = usePersonalTrends('facebook');
  ...
  const handleDetect = async () => {
    try {
      ...
      toast({ title: 'Trends analyzed', description: count > 0 ? `Detected ${count} trend${count !== 1 ? 's' : ''} in your Facebook data.` : 'No significant trends found yet.' });
    } catch {
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 rounded-lg bg-[#1877F2]/10">
              <Activity className="h-5 w-5 text-[#1877F2]" />
            </div>
            <h1 className="text-3xl font-bold text-foreground">Trend Intelligence</h1>
            <Badge variant="outline" className="text-xs border-[#1877F2]/30 text-[#1877F2] gap-1">
              <Facebook className="h-3 w-3" /> Facebook
            </Badge>
          </div>
          <p className="text-muted-foreground">AI-detected patterns in your Facebook page performance.</p>
        </motion.div>
      </div>
        ...
        <ChartCard title="No Trends Detected" subtitle="Run the AI analysis to discover patterns" delay={0.1}>
          <div className="text-center py-12">
            ...
            <p className="text-xs text-muted-foreground max-w-md mx-auto">
              Connect your Facebook page and click "Detect Trends" to let AI identify patterns like content performance shifts, audience growth trends, and engagement anomalies.
            </p>
          </div>
        </ChartCard>
      )}
        ...
        <div className="p-5">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Data-driven content ideas based on your Facebook trends</p>
            </div>
          </div>
          {trends.length > 0 ? (
            ...
            <div className="text-center py-6">
              ...
              <p className="text-sm text-muted-foreground">Connect Facebook and detect trends to generate AI-powered content strategies.</p>
            </div>
          )}
        </div>
      </motion.div>
