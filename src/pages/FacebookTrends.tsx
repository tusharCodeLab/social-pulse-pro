...
const trendTypeColors: Record<string, string> = {
  content: 'bg-[#1877F2]/20 text-[#1877F2]',
...
function formatConfidence(score: number): { label: string; className: string } {
  ...
  if (pct >= 75) return { label: `${pct}% · High`, className: 'text-[#1877F2]' };
  if (pct >= 50) return { label: `${pct}% · Moderate`, className: 'text-chart-impressions' };
  return { label: `${pct}% · Low`, className: 'text-muted-foreground' };
}

export default function FacebookTrends() {
  const { toast } = useToast();
  const { data: trends, isLoading: loadingTrends, error: trendsError } = usePersonalTrends('facebook');
  ...
  const handleDetectTrends = async () => {
    try {
      const result = await detectTrends.mutateAsync('facebook');
      const count = result?.trends?.length || 0;
      toast({
        title: 'Analysis complete',
        description: count > 0
          ? `Detected ${count} trend${count !== 1 ? 's' : ''} in your Facebook performance data.`
  ...
  const handleContentIdeas = async () => {
    try {
      ...
      toast({ title: 'Generation Failed', description: error instanceof Error ? error.message : 'Failed to generate ideas.', variant: 'destructive' });
    }
  };
      ...
      <div className="mb-8">
        ...
        >
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-lg bg-[#1877F2]/10"><Activity className="h-5 w-5 text-[#1877F2]" /></div>
            <div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {trends.map((trend, i) => {
              ...
              return (
                <motion.div key={trend.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.05 }}>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-foreground">{directionLabel[trend.direction] || 'Steady'}</span>
                        </div>
      ...
      >
        <div className="p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className="p-2.5 rounded-lg bg-gradient-to-br from-chart-impressions/20 to-[#1877F2]/20 border border-chart-impressions/30">
          ...
          {contentIdeas && contentIdeas.length > 0 ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
              {strategy && (
                <div className="p-3 rounded-lg bg-gradient-to-r from-chart-impressions/5 to-[#1877F2]/5 border border-chart-impressions/10">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                {contentIdeas.map((idea, i) => {
                  ...
                  return (
                    <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="p-4 rounded-lg bg-muted/20 border border-border/50">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <div className="flex items-center gap-1 text-xs text-foreground bg-muted/40 px-2 py-1 rounded-md"><FormatIcon className="h-3 w-3" />{idea.format}</div>
                        <div className="flex items-center gap-1 text-xs text-foreground bg-muted/40 px-2 py-1 rounded-md"><Calendar className="h-3 w-3" />{idea.bestDay}</div>
