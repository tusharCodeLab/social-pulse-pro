...
const POST_TYPE_LABELS: Record<string, { label: string; icon: typeof Image }> = {
  ...
  link: { label: 'Link Post', icon: Layers },
...
export default function FacebookPosts() {
  const { data: posts, isLoading: loadingPosts, error: postsError } = usePostsApi('facebook');
  const { data: stats, isLoading: loadingStats } = usePostStatsApi('facebook');
  const { data: engagementTrend, isLoading: loadingTrend } = useEngagementAnalyticsApi(14, 'facebook');
  ...
  const handleGenerateCaptions = async () => {
    ...
    try {
      ...
      toast({ title: 'Generation Failed', description: error instanceof Error ? error.message : 'Failed to generate captions.', variant: 'destructive' });
    }
  };
      ...
      <div className="mb-8">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 rounded-lg bg-[#1877F2]/10"><FileText className="h-5 w-5 text-[#1877F2]" /></div>
            <h1 className="text-3xl font-bold text-foreground">Posts Analysis</h1>
          ...
          <p className="text-muted-foreground">
            Track and analyze the performance of your Facebook posts.
      ...
      {postsError ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <EnhancedMetricCard label="Total Posts" value={stats?.totalPosts.toString() || '0'} icon={FileText} delay={0.1} color="hsl(214,89%,52%)" />
          ...
          <motion.div
            ...
            className="mb-8 rounded-xl border border-[#1877F2]/20 bg-gradient-to-r from-card via-card to-[#1877F2]/5 overflow-hidden"
            style={{ boxShadow: '0 4px 30px -8px hsl(214 89% 52% / 0.15)' }}
          >
            <div className="p-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 rounded-lg bg-gradient-to-br from-[#1877F2]/20 to-primary/20 border border-[#1877F2]/30">
                    <Wand2 className="h-5 w-5 text-[#1877F2]" />
                  </div>
                  <div>
                ...
                <Button
                  size="sm" onClick={handleCoach}
                  disabled={aiCoach.isPending || (stats?.totalPosts || 0) === 0}
                  className="gap-1.5 bg-gradient-to-r from-[#1877F2] to-[#1877F2]/80 hover:from-[#1877F2]/90 hover:to-[#1877F2]/70 text-white"
                >
              ...
              {coaching ? (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                    ...
                    <div className="flex items-center gap-4 p-4 rounded-lg bg-muted/30 border border-border/50">
                      <div className="relative w-14 h-14 flex-shrink-0">
                        <svg className="w-14 h-14 -rotate-90" viewBox="0 0 48 48">
                          ...
                          <circle cx="24" cy="24" r="20" fill="none" stroke="hsl(214,89%,52%)" strokeWidth="4"
                            strokeDasharray={`${(coaching.overallScore / 100) * 125.6} 125.6`} strokeLinecap="round" />
                        </svg>
                        <span className="absolute inset-0 flex items-center justify-center text-sm font-bold text-[#1877F2]">{coaching.overallScore}</span>
                      </div>
                      <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="p-4 rounded-lg bg-muted/20 border border-border/50">
                      <div className="flex items-center gap-1.5 mb-3"><Lightbulb className="h-4 w-4 text-chart-impressions" /><span className="text-sm font-semibold text-foreground">Caption Tips</span></div>
                      <div className="space-y-2">
                        {coaching.captionTips.slice(0, 5).map((tip, i) => (
                          <div key={i} className="flex items-start gap-2"><ChevronRight className="h-3.5 w-3.5 text-[#1877F2] mt-0.5 flex-shrink-0" /><p className="text-xs text-muted-foreground">{tip}</p></div>
                        ))}
                      </div>
                    </div>
                    <div className="p-4 rounded-lg bg-muted/20 border border-border/50">
                      <div className="flex items-center gap-1.5 mb-3"><Hash className="h-4 w-4 text-chart-reach" /><span className="text-sm font-semibold text-foreground">Hashtag Ideas</span></div>
                    ...
                    <div className="p-4 rounded-lg bg-muted/20 border border-border/50">
                      <div className="flex items-center gap-1.5 mb-3"><Brain className="h-4 w-4 text-[#1877F2]" /><span className="text-sm font-semibold text-foreground">Content Ideas</span></div>
                      <div className="space-y-2">
                        {coaching.contentIdeas.slice(0, 5).map((idea, i) => (
                          <div key={i} className="flex items-start gap-2"><ChevronRight className="h-3.5 w-3.5 text-chart-reach mt-0.5 flex-shrink-0" /><p className="text-xs text-muted-foreground">{idea}</p></div>
                        ))}
                      </div>
                    </div>
                  </div>
                  <div className="mt-4 p-3 rounded-lg bg-gradient-to-r from-[#1877F2]/5 to-chart-reach/5 border border-[#1877F2]/10">
                    <p className="text-xs text-center text-muted-foreground">
                      <Sparkles className="h-3.5 w-3.5 text-[#1877F2] inline mr-1" />
                ...
                <p className="text-sm text-muted-foreground text-center py-4">
                  {(stats?.totalPosts || 0) === 0
                    ? 'No posts available. Connect your Facebook Page in Settings to import posts.'
          ...
          >
            <div className="p-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 rounded-lg bg-gradient-to-br from-chart-reach/20 to-[#1877F2]/20 border border-chart-reach/30">
              ...
              <div className="flex gap-2 mb-4">
                <Input
                  placeholder="Enter topic or theme (e.g., 'product launch', 'community update')..."
                  value={captionTopic}
              ...
              {captions && captions.length > 0 ? (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
                  {captions.map((cap, i) => (
                    <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="p-4 rounded-lg bg-muted/20 border border-border/50 group">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="secondary" className={`text-xs ${cap.estimatedEngagement === 'Very High' ? 'bg-chart-sentiment-positive/10 text-chart-sentiment-positive' : cap.estimatedEngagement === 'High' ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>
                        ...
                        <Button size="sm" variant="ghost" className="gap-1.5 h-7 text-xs opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => handleCopyCaption(cap.caption, i)}>
                ...
                {trendData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={trendData}>
                      <defs>
                        <linearGradient id="colorLikesFb" x1="0" y1="0" x2="0" y2="1">
                      ...
                      <Area type="monotone" dataKey="likes" stroke={COLORS.likes} fill="url(#colorLikesFb)" strokeWidth={2} />
                    </AreaChart>
                  </ResponsiveContainer>
                ) : (
          ...
          <ChartCard title="Top Performing Posts" subtitle={`${topPosts.length} posts ranked by engagement`} delay={0.4}>
            {topPosts.length > 0 ? (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border">
                      ...
                      <th className="text-right py-3 px-4 text-xs font-medium text-muted-foreground uppercase tracking-wider">Shares</th>
                        ...
                        >
                          ...
                          <td className="py-3 px-4">
                            <div className="flex items-start gap-3">
                              {post.mediaUrl ? (
                                <img src={post.mediaUrl} alt="" loading="lazy" onError={(e) => { e.currentTarget.style.display = 'none'; e.currentTarget.nextElementSibling?.classList.remove('hidden'); }} className="w-12 h-12 rounded-lg object-cover border border-border flex-shrink-0" />
                              ) : null}
                              ...
                              <div className="min-w-0">
                                <p className="text-sm text-foreground line-clamp-2 max-w-[280px]">{formatPostContent(post.content, post.type)}</p>
                                <p className="text-xs text-muted-foreground mt-1">{formatRelativeDate(post.publishedAt)}</p>
                              </div>
                            </div>
                          </td>
                          <td className="py-3 px-4 text-right">
                            <div className="flex items-center justify-end gap-1"><Heart className="h-3 w-3 text-muted-foreground" /><span className="text-sm text-foreground">{formatNumber(post.metrics.likes)}</span></div>
                          </td>
                          <td className="py-3 px-4 text-right">
                            <div className="flex items-center justify-end gap-1"><MessageCircle className="h-3 w-3 text-muted-foreground" /><span className="text-sm text-foreground">{formatNumber(post.metrics.comments)}</span></div>
                          </td>
                          <td className="py-3 px-4 text-right">
                            <div className="flex items-center justify-end gap-1"><Share2 className="h-3 w-3 text-muted-foreground" /><span className="text-sm text-foreground">{formatNumber(post.metrics.shares)}</span></div>
                          </td>
                          <td className="py-3 px-4 text-right">
              ...
              <div className="py-12 text-center">
                ...
                <p className="text-sm text-muted-foreground mt-1">Connect your Facebook Page in Settings to import posts.</p>
              </div>
            )}
          </ChartCard>
        </>
